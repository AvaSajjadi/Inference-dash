...
# IMPORTANT:
# Do NOT declare ModelBase::set_signal here.
# If it isn't defined/linked in the C++ library, it will cause "undefined symbol" at import time.
...
cdef extern from "../core/include/ModelORNOR.h" namespace "nlb":
    cdef cppclass ModelORNOR:
        ...
        # Signature must match your C++ header exactly (includes base_seed as last arg)
        ModelORNOR(
            const network_t,
            const evidence_dict_t,
            const prior_active_tf_set_t,
            const double[9],
